...
def t_COINS(t):
    r'[a-zA-Z_][a-zA-Z_0-9]*[ ]*is[ ]*number[ ]*of[ ]*coins'
    t.value = 'COINS'
    return t
...
def p_command(p):
    ...
    elif len(p) == 2 and p[1] == 'COINS':
        value = grid.getNumberOfCoins(bot.myX(), bot.myY())
        print("Controller.py",value)
        commandStack.append(makeSingleCommand("getNumberOfCoins(bot.myX(), bot.myY())"))
    elif len(p) == 3:
        bot.move(p[2])
